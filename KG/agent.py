...
class Neo4jAgent:
    def __init__(self, memory_enabled: bool = False, memory_path: str | None = None, memory_max: int = 100):
        self.uri = "neo4j+s://cd477924.databases.neo4j.io"
        self.user = "neo4j"
        self.password = "RZbkJ7D1h9qp4HdiVKK1l8K3Y5I3tZjnwF939p0Uoz0"
        self.database = "neo4j"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Exécuter des requêtes en langage naturel sur le graphe Neo4j")
    parser.add_argument("--query", "-q", help="Requête unique en langage naturel à exécuter")
    parser.add_argument("--file", "-f", help="Fichier contenant une requête par ligne (lignes commençant par # ignorées)")
    args = parser.parse_args()

    agent = Neo4jAgent(memory_enabled=bool(os.getenv('AGENT_MEMORY', '1') != '0') or False,
                       memory_path=os.getenv('AGENT_MEMORY_PATH', None))
    try:
        queries = []
        if args.query:
            ...
            if not os.path.exists(path):
                raise FileNotFoundError(f"Fichier de requêtes introuvable: {path}")
            ...
            print("Mode interactif (Entrée vide pour quitter)...")
            while True:
                try:
                    q = input("Query> ")
                except EOFError:
                ...
                print(f"\nProcessing query: {q}")
                try:
                    formatted_result = agent.execute_query(q)
                    print("Réponse:")
