...
class DevisRequest(BaseModel):
    ...
    @validator('valeur_venale', 'valeur_a_neuf', 'capital_bris_de_glace', 'capital_dommage_collision')
    def must_be_positive(cls, v):
        if v <= 0:
            raise ValueError('Doit être supérieur à 0')
        return v

    @validator('nombre_place')
    def places_positive(cls, v):
        if v < 1:
            raise ValueError('Nombre de places doit être >= 1')
        return v

    @validator('puissance')
    def puissance_range(cls, v):
        if v < 1 or v > 6:
            raise ValueError('Puissance doit être entre 1 et 6')
        return v

    @validator('classe')
    def classe_positive(cls, v):
        if v < 1:
            raise ValueError('Classe doit être >= 1')
        return v
...
async def handle_devis_request(params: dict):
    ...
    try:
        ...
        return {"error": f"Erreur lors de la récupération du devis: {str(e)}"}
    ...
    # Packs
    for pack in devis_data.get('body', {}).get('result', []):
        ...
        total_prime = f"{float(pack.get('montantTotalPrime', 0)):.3f} TND" if pack.get('montantTotalPrime') else "-"
        monthly_prime = f"{float(pack.get('montantPrimeDivisePar12', 0)):.3f} TND" if pack.get('montantPrimeDivisePar12') else "-"
        ...
        for g in guarantees:
            table_data.append([
                g.get('libGarantie', 'N/A'),
                f"{float(g.get('capital',0)):.3f} TND" if g.get('capital') else "-",
                g.get('codeFranchise', '-') or "-",
                g.get('codeGarantie', 'N/A')
            ])
